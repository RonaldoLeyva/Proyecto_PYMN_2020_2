...
float x1,x2,x3,E1,E2,E3,x11,x22,x33; 
int i,j;
int main()
{
for(j=0;j<5;j++)
{
    if(j==0){ 
        ...
        x22 = x2;
        x2 = ((-A[1][0]*x11)-(A[1][2]*x3)+b[1])/A[1][1];
        E2=fabs((x2-x22)/(x2))*100;
        x33 = x3;
        x3 = ((-A[2][0]*x11)-(A[2][1]*x22)+b[2])/A[2][2];
        E3=fabs((x3-x33)/(x3))*100;
        printf("x1 iteracion %d es igual a %f error del %f %%\n",j+1,x1,E1);
        printf("x2 iteracion %d es igual a %f error del %f %%\n",j+1,x2,E2);
        printf("x3 iteracion %d es igual a %f error del %f %%\n",j+1,x3,E3);
        }
    }
}
